...
If (isAUFresh := checkAUFreshness())	; There is either an update or file 'AkelUpdater.original.exe' doesn't yet exist.
{
	If (isAUFresh = 1)
	{
...
checkAUFreshness()
{
	...
	{
		RunWait, AkelUpdater.original.exe
		WinWait, ahk_exe AkelUpdater.original.exe
		WinGetTitle, titleAU
		IfInString, titleAU, "Обновление"
			Return, 1
	}
	Else
		Return, 2
